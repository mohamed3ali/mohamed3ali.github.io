...
export default function HtmlMinifier() {
  ...
  const seoData = getSeoData('html-minifier');
  ...
  const handleMinify = async () => {
    ...
    try {
      const response = await toolsAPI.htmlMinifier(input);
      ...
      setError(err.response?.data?.error || 'Failed to minify HTML');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Head>
        <title>HTML Minifier - Modern Tools</title>
        <meta name="description" content="Minify HTML code to reduce file size" />
      </Head>
          ...
          >
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-blue-400 to-purple-600 text-transparent bg-clip-text">
                {t('tools.htmlMinifier')}
              </h1>
              <p className="text-gray-300 text-lg">
                {t('descriptions.htmlMinifier')}
              </p>
            </div>
  ...
  <body>
    <h1>Hello World</h1>
            ...
            <div className="mt-16 max-w-4xl mx-auto">
              <h2 className="text-2xl font-bold mb-6 text-center">Why Minify HTML?</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gray-800/50 backdrop-blur-sm p-6 rounded-lg border border-gray-700">
                  <div className="text-blue-400 text-2xl mb-3">⚡</div>
                  <h3 className="text-lg font-semibold mb-2">Faster Loading</h3>
                  <p className="text-gray-400 text-sm">
                    Reduce file size for faster page load times
                  </p>
                </div>
                <div className="bg-gray-800/50 backdrop-blur-sm p-6 rounded-lg border border-gray-700">
                  <div className="text-purple-400 text-2xl mb-3">💾</div>
                  <h3 className="text-lg font-semibold mb-2">Save Bandwidth</h3>
                  <p className="text-gray-400 text-sm">
                    Lower data transfer costs and requirements
                  </p>
                </div>
                <div className="bg-gray-800/50 backdrop-blur-sm p-6 rounded-lg border border-gray-700">
                  ...
                  <p className="text-gray-400 text-sm">
                    Improve search rankings with optimized code
                  </p>
                </div>
              </div>
            </div>
