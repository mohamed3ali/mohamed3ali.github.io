...
export default function AddPageNumbers() {
  ...
  const seoData = getSeoData('add-page-numbers');
  ...
  const handleAddNumbers = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/tools/pdf-add-page-numbers', {
        method: 'POST',
        body: formData
      });
      ...
      if (data.success) {
        ...
        a.href = url;
        a.download = data.filename || 'numbered.pdf';
        a.click();
        setError('');
      } else {
        setError(data.error || 'Adding page numbers failed');
      }
    } catch (err) {
  ...
  return (
    <>
      <SEOHead toolSlug="add-page-numbers" />
          ...
          >
            <div className="text-center mb-12">
              <h1 className="text-4xl md:text-5xl font-bold mb-4 bg-gradient-to-r from-blue-400 to-purple-600 text-transparent bg-clip-text">
                {t('tools.addPageNumbers')}
              </h1>
              <p className="text-gray-300 text-lg">
                {t('descriptions.addPageNumbers')}
              </p>
            </div>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg p-6 mb-6">
                <h3 className="text-lg font-semibold mb-4">Page Number Settings</h3>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="roman-small">i, ii, iii...</option>
                    </select>
                  </div>
                </div>
              </div>
